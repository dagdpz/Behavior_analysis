function [out_stat, anova_out]= bh_statistics_control_exp_group(batch,testing)
global GLO
anova_out=[];
                        ...
                        if ~any(isempty(real(x)) | isempty(real(y)))
                            if ~all(isnan(real(x))) && ~all(isnan(real(y)))
                                if GLO.calculate_statististics && ~GLO.one_subject
                                    ...
                                    if isempty(strfind(par,'sac_rea')) && GLO.paired_testing_when_possible && (strcmp(subcondition,'raw_of_mean') || strcmp(subcondition,'raw_of_std' )) &&  numel(x)==numel(y)
                                        if (strcmp(testing,'groups')) && ~GLO.parametric_testing
                                            [pr hr tab_r]= signrank(real(x),real(y));
                                            [pi hi tab_i]= signrank(imag(x),imag(y));
                                        elseif (strcmp(testing,'groups')) && GLO.parametric_testing
                                            [hr pr cir tab_r]= ttest(real(x),real(y),0.05,0,1);
                                            [hi pi cii tab_i]= ttest(imag(x),imag(y),0.05,0,1);
                                        end
                                        
                                    elseif isempty(strfind(par,'sac_rea')) %| ~strcmp(par,'accuracy')
                                        if (strcmp(testing,'groups')) && ~GLO.parametric_testing
                        ...
                        if ~all(isnan(x)) && ~all(isnan(y))
                            if GLO.calculate_statististics && ~GLO.one_subject
                                if isempty(strfind(par,'sac_rea')) %% strcmp(par,'sac_rea')
                                    ...
                                    elseif GLO.paired_testing_when_possible && (strcmp(subcondition,'raw_of_mean') || strcmp(subcondition,'raw_of_std' )) &&  numel(x)==numel(y)
                                        if (strcmp(testing,'groups')) && ~GLO.parametric_testing
                                            [pp h tab]= signrank(x,y);
                                        elseif (strcmp(testing,'groups')) && GLO.parametric_testing
                                            [h pp ci tab]= ttest(x,y,0.05,0,1);
                                        end
                                    else
                                        if (strcmp(testing,'groups')) && ~GLO.parametric_testing
