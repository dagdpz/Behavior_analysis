...
if recalibrated==1
    keys.path_to_save='Y:\Projects\Pulv_eye_gaze_position\behavior\recalibrated';
else
    keys.path_to_save='Y:\Projects\Pulv_eye_gaze_position\behavior\notrecalibrated';
...
for monkeys={'Cur','Lin','Both'}
    monkey=monkeys{:};
    mainpath='C:\Users\lschneider\Desktop\gaze_data';
    if strcmp(monkey,'Both')
        monkey='Lin';
        taskfield=[monkey '_dPulv_PT0_Msac_mov'];
    ...
    positions=unique([all_saccades.tar_pos] - [all_saccades.fix_pos]);
    ...
    for s=1:numel(data)
        ...
        for f=1:numel(fixations)
            ...
            for p=1:numel(positions)
                tr_idx=([saccades.tar_pos] - fix_pos)==positions(p) & real([saccades.fix_pos])==fixations(f);
