function [out_stat] = bh_statistics(batch,testing)
global GLO
                                    ...
                                    if ~any(isempty(real(x)) | isempty(real(y)))
                                        if ~all(isnan(real(x))) && ~all(isnan(real(y)))
                                            if GLO.calculate_statististics && ~GLO.one_subject
                                                if GLO.paired_testing_when_possible && (strcmp(subcondition,'raw_of_mean') || strcmp(subcondition,'raw_of_std' )) &&  numel(x)==numel(y)
                                                    if ~GLO.parametric_testing
                                                        [pr hr tab_r]= signrank(real(x),real(y));
                                                        [pi hi tab_i]= signrank(imag(x),imag(y));
                                                    elseif GLO.parametric_testing
