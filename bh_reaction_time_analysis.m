function [f_input, out_comp, out_stru_ext, unique_pos] = bh_reaction_time_analysis(files_for_input,dates, batching,subject_files,steady)
global  GLO
GLO.dates                           = dates;
%% ??
...
if GLO.delete_last
    for idx_batch                   = 1:numel(files_for_input_batch(:,1))
        DAG_delete_last_empty_trial(files_for_input_batch{idx_batch,1});
    end
end
...
%{'correct_offset',0,'success',1,'display',0,'keep_raw_data',1,'saccade_definition',4,'reach_1st_pos',1,'correlation_conditions',{}};
