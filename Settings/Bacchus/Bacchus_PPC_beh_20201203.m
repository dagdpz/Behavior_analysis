...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Simultaneous_dPul_PPC_recordings\behavior\beh_analysis\Bacchus\20201203';
...
group{1}                        = repmat({'Bacchus'},1,1);
dates_subject_in{1}             = repmat({20201203},1,1);;
batching{1}.runs                = {3};  % either empty or specific runs specified
batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

subject_ID{2}='Experimental';
group{2}                        = repmat({'Bacchus'},1,3);
dates_subject_in{2}             = repmat({20201203},1,3);;
batching{2}.runs                = {4;5;6};  % either empty or specific runs specified
