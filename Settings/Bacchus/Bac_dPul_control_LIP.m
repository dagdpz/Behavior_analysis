...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Simultaneous_dPul_PPC_recordings\behavior\beh_analysis\Bacchus\Bac_dPul_control_LIP';
GLO.type_of_free_gaze               =   '6';
GLO.one_subject                     =   0;
GLO.trial_by_trial                  =   0; % for statistics, 0 means calculate statistics bases on average per run
% GLO.trial_by_trial_traces           =   0;
...
%  load('Y:\Projects\Simultaneous_dPul_PPC_recordings\ephys\dPul_control_LIP_Bac_7_sessions\behaviour_filelist.mat');
%  
%  filelist_formatted_control=filelist_formatted.Bac_LIP_R_PT0_Dsa_han;
%  filelist_formatted_inactivation=filelist_formatted.Bac_LIP_R_PT1_Dsa_han;

subject_ID{1}='Control';
group{1}                        = repmat({'Bacchus'},1,7);
dates_subject_in{1}             = {20201209,20210127,20210203,20210205,20210317,20210325,20210401};
batching{1}.runs                = {2;2;3;2;2;2;2}; % either empty or specific runs specified
batching{1}.inactivation_sites   = {'R','R','R','R','R','R','R'};
batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;


subject_ID{2}='Experimental';
group{2}                        = repmat({'Bacchus'},1,7);
dates_subject_in{2}             = {20201209,20210127,20210203,20210205,20210317,20210325,20210401};
batching{2}.runs                = {3;3;4;3;3;3;3};  % either empty or specific runs specified
batching{2}.inactivation_sites  = {'R','R','R','R','R','R','R'};
