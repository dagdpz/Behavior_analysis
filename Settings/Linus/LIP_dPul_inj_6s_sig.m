...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Simultaneous_dPul_PPC_recordings\behavior\beh_analysis\Linus\LIP_dPul_inj_6s_sig';
...
GLO.summary                         =   [1 2 5 10 11]; %which plots (vector of number of -1 for all plots) [1 2 3 5 10]
...
% group{1}                        = repmat({'Linus'},1,7);
% dates_subject_in{1}             = {20210520,20210610,20210616,20210709,20210901,20211006,20211021};
% batching{1}.runs                = {2;3;2;2;2;2;2};  % either empty or specific runs specified
% batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
% batching{1}.range_of_dates      = 0;

subject_ID{1}='Control';
group{1}                        = repmat({'Linus'},1,6);
dates_subject_in{1}             = {20210610,20210709,20211006,20211126,20211201,20211208};
batching{1}.runs                = {3;2;2;2;2;2};  % either empty or specific runs specified
batching{1}.inactivation_sites   = {'R','R','R','L','L','L'};
batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;


% subject_ID{2}='Experimental';
% group{2}                        = repmat({'Linus'},1,7);
% dates_subject_in{2}             = {20210520,20210610,20210616,20210709,20210901,20211006,20211021};
% batching{2}.runs                = {[3 4];[4 5];[3 4 5];3;[3 4 5];[3 4 5];[4 5 6]};  % either empty or specific runs specified
% batching_type{2}                = 1; % 1 run by run, 2 session by session, 3 group by group
% batching{2}.range_of_dates      = 0;

subject_ID{2}='Experimental';
group{2}                        = repmat({'Linus'},1,6);
dates_subject_in{2}             = {20210610,20210709,20211006,20211126,20211201,20211208};
batching{2}.runs                = {[5];[3];[4];3;[3];[3]};  % either empty or specific runs specified
batching{2}.inactivation_sites   = {'R','R','R','L','L','L'};
batching_type{2}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{2}.range_of_dates      = 0;
