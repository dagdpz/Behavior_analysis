...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Simultaneous_dPul_PPC_recordings\behavior\beh_analysis\Linus\LIP_dPul_inj_3s_L';
...
group{1}                        = repmat({'Linus'},1,3);
dates_subject_in{1}             = {20211126,20211201,20211208};
batching{1}.runs                = {2;2;2};  % either empty or specific runs specified
batching{1}.inactivation_sites   = {'L','L','L'};
batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

subject_ID{2}='Experimental';
group{2}                        = repmat({'Linus'},1,3);
dates_subject_in{2}             = {20211126,20211201,20211208};
batching{2}.runs                = {[3];[3];[3]};  % either empty or specific runs specified
batching{2}.inactivation_sites   = {'L','L','L'};
