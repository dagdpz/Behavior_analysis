...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Simultaneous_dPul_PPC_recordings\behavior\beh_analysis\Linus\20210526';
...
dates_subject_in{1}             = repmat({20210526},1,1);;
batching{1}.runs                = {7};  % either empty or specific runs specified
batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

% subject_ID{2}='Experimental';
% group{2}                        = repmat({'Linus'},1,2);
% dates_subject_in{2}             = repmat({20210520},1,2);;
