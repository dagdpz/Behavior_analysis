...
GLO.append_pdfs                     =   0;
GLO.parent_folder                   =   '';
GLO.folder_to_save                  =   'Y:\Projects\Simultaneous_dPul_PPC_recordings\behavior\beh_analysis\Linus\5s';
...
group{1}                        = repmat({'Linus'},1,5);
dates_subject_in{1}             = {20210520,20210610,20210616,20210709,20210901};
batching{1}.runs                = {2;3;2;2;2};  % either empty or specific runs specified
batching_type{1}                = 1; % 1 run by run, 2 session by session, 3 group by group
batching{1}.range_of_dates      = 0;

subject_ID{2}='Experimental';
group{2}                        = repmat({'Linus'},1,5);
dates_subject_in{2}             = {20210520,20210610,20210616,20210709,20210901};
batching{2}.runs                = {4;5;4;3;5};  % either empty or specific runs specified
