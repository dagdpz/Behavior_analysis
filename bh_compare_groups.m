function bh_compare_groups(batch,testing)
global GLO
close all
...
for idx=1:numel(bars)
    ...
    if GLO.text_in_plot
        t=text(idx+(0.1), d.(tmean)-(d.(tmean)*0), sprintf('%.4f + %.4f',d.(tmean),d.(tsem)));
...
for g=1:size(groups,1)
    ...
    for p=1:numel(current_group)
        ...
        for d=1:numel(conditions)
            ...
            if GLO.text_in_plot
                t=text(idx+(0.1), temp_mean-(temp_mean*0), sprintf('%.4f + %.4f',temp_mean,temp_sem));
                set(t,'Color', [0.3 0.3 0.3], 'FontSize', GLO.fontsize_small, 'FontWeight', 'bold')
            end
            if GLO.hits_in_plot
...
for g=1:size(groups,1)
    ...
    %     id_x=id_x+0.1;
        ...
        for d=1:numel(conditions)
            ...
            [slo, int, c_R, c_P] = bh_myregr_eye_hand(temp_rea',temp_sac',0,GLO.remove_outliers);
